#run_sim_analytical_compare_load_trace_new.py
...
def run_simulator (argument_tuple) :
    ...
    if gdb == 0:
        command = "./waf --run=\"" + \
            ...
            "--tracePath=" + str(config_data['SimulationParamsInputs']['tracePath']) + " " + \
            "--tag=" + tag + \
        "\" > " + output_filename
        print(command)
        os.system(command)
    ...
    if gdb == 1:
        command= './waf --run=\"' + \
            config_data['SimulationParamsInputs']['programFile'] + '" --command-template="gdb --args %s ' \
            "--randSeed=" + str(randomSeed) + " " + \
            "--queueSize=" + str(config_data['SimulationParamsInputs']['queueSize']) + " " + \
            "--ecmp=" + str(config_data['DCNStructure']['ecmpRouting']) + " " + \
            additional_parameters + " " + \
