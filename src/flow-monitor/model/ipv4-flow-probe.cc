  ...
  if (m_classifier->Classify (ipHeader, ipPayload, &flowId, &packetId))
    {
      ...
      NS_LOG_DEBUG ("ReportFirstTx ("<<this<<", "<<flowId<<", "<<packetId<<", "<<size<<"); "
                                     << ipHeader << *ipPayload);
      Address srcAddress = ipHeader.GetSource();
      Address desAddress = ipHeader.GetDestination();     
      uint8_t data[4];
      ...
      srcPort <<= 8;
      srcPort |= data[1];                          
      m_flowMonitor->ReportFirstTx (this, flowId, packetId, size, interface, srcAddress, desAddress, srcPort);
  ...
  if (found)
    {
      ...
      Address srcAddress = ipHeader.GetSource();
      Address desAddress = ipHeader.GetDestination(); 
      m_flowMonitor->ReportForwarding (this, flowId, packetId, size, interface, srcAddress, desAddress);
    }
}
  ...
  if (found)
    {
      ...
      Address srcAddress = ipHeader.GetSource();
      Address desAddress = ipHeader.GetDestination(); 
      m_flowMonitor->ReportLastRx (this, flowId, packetId, size, interface, srcAddress, desAddress);
    }
}
...
Ptr<Node>
Ipv4FlowProbe::GetNode (void) const
{
  return m_node;
}
