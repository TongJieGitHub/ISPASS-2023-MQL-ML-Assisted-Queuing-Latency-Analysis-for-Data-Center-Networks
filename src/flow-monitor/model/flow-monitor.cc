...
#include <sstream>
#include "ns3/node.h"
#include "ns3/flow-log.h"
#include "ns3/queue-log.h"
...
FlowMonitor::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::FlowMonitor")
    ...
    .AddAttribute ("FlowInterruptionsMinTime", ("The minimum inter-arrival time that is considered a flow interruption."),
                   ...
                   MakeTimeChecker ())
    .AddTraceSource ("PacketDelay",
                  "End to end delay of packet",
                  MakeTraceSourceAccessor (&FlowMonitor::m_tracePacketDelay),
  ...
  : m_enabled (false)
{
  ...
  m_ofstream.open("log.csv", std::ios::out);
}
...
void
FlowMonitor::ReportFirstTx (Ptr<FlowProbe> probe, uint32_t flowId, uint32_t packetId, uint32_t packetSize, uint32_t interface, Address srcAddress, Address desAddress, uint16_t srcPort)
{
  ...
  tracked.probe = probe;
  tracked.nodeId = probe->GetNode()->GetId();
  tracked.interface = interface;
  ...
  // m_flowlogv2.m_TimeWarmUp = this->m_TimeWarmUp;
  ...
  if (now >= m_TimeWarmUp)
  {
    if (enable_first_pod)
    {
      ...
      if (client < numPods * numPods / 4 || server < numPods * numPods / 4)
      {
        if (enable_log)
        {
          m_flowlogv2.CreateLog(probe, interface, 0, flowId, srcAddress, desAddress, address_client_map, address_server_map);
          m_flowlogv2.UpdateLog(probe, interface, flowId, srcAddress, desAddress, packetId, packetSize, 0, false);
          // m_flowlogv2.sourceDataRate = this->sourceDataRate;
          // m_flowlogv2.filename = this->filename;
          // m_flowlogv2.tag = this->tag;
          // m_flowlogv2.m_TimeWindow = this->m_TimeWindow;
        }
        if (enable_mimic_log)
        {
          m_mimiclogv2.CreateLog(probe, interface, flowId, srcAddress, desAddress, srcPort, address_client_map, address_server_map);
          m_mimiclogv2.UpdateLog(probe, interface, flowId, srcAddress, desAddress, srcPort, packetId, packetSize, 0);
          // m_mimiclogv2.filename = this->filename;
          // m_mimiclogv2.tag = this->tag;
          // m_mimiclogv2.m_TimeWindow = this->m_TimeWindow;
        }
        if (enable_packet_log)
        {
          m_ofstream
              ...
              << 0 << ","
              << 0 << ","
              << m_flowlogv2.GetCount(probe, interface, flowId) << std::endl;
        }
      }
    }
    else
...
void
FlowMonitor::ReportForwarding (Ptr<FlowProbe> probe, uint32_t flowId, uint32_t packetId, uint32_t packetSize, uint32_t interface, Address srcAddress, Address desAddress)
{
  NS_LOG_FUNCTION (this << probe << flowId << packetId << packetSize);
  if (!m_enabled)
    {
  ...
  // Time now = Simulator::Now ();
  // std::ofstream of;
  // Ptr<Node> node = probe->GetNode();
  // of.open("flow_" + std::to_string(flowId) + "_Queueing_" + std::to_string(tracked->second.timesForwarded + 1) + "_node_" + std::to_string(node->GetId()) + ".tr", std::ios::out | std::ios::app);
  // of << now.GetNanoSeconds() << " " << packetId << " "<< packetSize << " " << (now.GetNanoSeconds() - tracked->second.lastSeenTime.GetNanoSeconds()) << std::endl;
  // of.close();
  Time now = Simulator::Now ();
  int64_t latency = now.GetNanoSeconds() - tracked->second.lastSeenTime.GetNanoSeconds();
  uint32_t stage = tracked->second.timesForwarded + 1;
  ...
  if (now >= m_TimeWarmUp)
  {
    if (enable_first_pod)
    {
      ...
      if (client < numPods * numPods / 4 || server < numPods * numPods / 4)
      {
        if (enable_log)
        {
          m_flowlogv2.CreateLog(probe, interface, stage, flowId, srcAddress, desAddress, address_client_map, address_server_map);
          m_flowlogv2.UpdateLog(probe, interface, flowId, srcAddress, desAddress, packetId, packetSize, latency, false);
          m_flowlogv2.UpdateLatency(tracked->second.probe, tracked->second.interface, flowId, srcAddress, desAddress, packetId, packetSize, latency);
        }
        if (enable_packet_log)
        {
          m_ofstream
  ...
  tracked->second.probe = probe;
  tracked->second.nodeId = probe->GetNode()->GetId();
  tracked->second.interface = interface;
...
void
FlowMonitor::ReportLastRx (Ptr<FlowProbe> probe, uint32_t flowId, uint32_t packetId, uint32_t packetSize, uint32_t interface, Address srcAddress, Address desAddress)
{
  NS_LOG_FUNCTION (this << probe << flowId << packetId << packetSize);
  if (!m_enabled)
    {
  ...
  if (now >= m_TimeWarmUp)
  {
    if (enable_first_pod)
    {
      ...
      if (client < numPods * numPods / 4 || server < numPods * numPods / 4)
      {
        ...
        if (enable_mimic_log)
        {
          m_mimiclogv2.UpdateLatency(tracked->second.probe, interface, flowId, srcAddress, desAddress, packetId, packetSize, delay.GetNanoSeconds());
        }
        if (enable_packet_log)
        {
          m_ofstream
                         ...
                         uint32_t reasonCode)
{
  ...
  if (enable_log)
  {
    // m_queuelogv2.UpdateQueueDrop(tracked->second.probe, tracked->second.interface, packetSize);
  }
  //std::cout << "dropped:" << tracked->second.probe->GetNode()->GetId() << "," << tracked->second.interface << ": " << packetSize << ": " << reasonCode << std::endl;
...
FlowMonitor::StartRightNow ()
{
  ...
  m_flowlogv2.m_TimeWindow = this->m_TimeWindow;
  m_mimiclogv2.m_TimeWindow = this->m_TimeWindow;
  m_flowlogv2.Schedule();
  m_mimiclogv2.Schedule();
}
