...
#include "ns3/flow-log-v2.h"
#include "ns3/queue-log.h"
#include "ns3/queue-log-v2.h"
#include "ns3/mimic-log.h"
#include "ns3/mimic-log-v2.h"
...
public:
  ...
  void ReportFirstTx (Ptr<FlowProbe> probe, FlowId flowId, FlowPacketId packetId, uint32_t packetSize, uint32_t interface, Address srcAddress, Address desAddress, uint16_t srcPort);
  /// FlowProbe implementations are supposed to call this method to
  ...
  /// \param packetId Packet ID
  /// \param packetSize packet size
  void ReportForwarding (Ptr<FlowProbe> probe, FlowId flowId, FlowPacketId packetId, uint32_t packetSize, uint32_t interface, Address srcAddress, Address desAddress);
  /// FlowProbe implementations are supposed to call this method to
  ...
  /// \param packetId Packet ID
  /// \param packetSize packet size
  void ReportLastRx (Ptr<FlowProbe> probe, FlowId flowId, FlowPacketId packetId, uint32_t packetSize, uint32_t interface, Address srcAddress, Address desAddress);
  /// FlowProbe implementations are supposed to call this method to
...
private:
  ...
  int64_t startTime = 0;
  FlowLog m_flowlog;
  FlowLogV2 m_flowlogv2;
  QueueLog m_queuelog;
  QueueLogV2 m_queuelogv2;
  MimicLog m_mimiclog;
  MimicLogV2 m_mimiclogv2;
