...
public:
  ...
  class FlowPerQueueLogTracker: public Object {
  public:
      uint32_t nodeId;
      uint32_t interface;
      uint32_t nodeIdFrom;
      uint32_t interfaceFrom;
...
private:
  ...
  // key: <<nodeID, interfaceID>, <nodeIDFrom, interfaceIDFrom>>
  std::map<std::pair<std::pair<uint32_t, uint32_t>, std::pair<uint32_t, uint32_t>>, Ptr<FlowPerQueueLogTracker>> m_flow_per_queue_logs;
