...
int main(int argc, char *argv[])
{
	...
	// Link parameters
	string unitsDataRate = "Mbps";     // Mbps, Gbps
	string linkBandwidth = "100";
	...
	// int avgPacketSize = 1024;
	// int minPacketSize = 512;
	// int maxPacketSize = 1536;
	int avgPacketSize = 32;   // over write via command line
	...
	cmd.AddValue("netProtocol", "ns3::UdpSocketFactory or ns3::TcpSocketFactory, (default TCP)", netProtocol);
	...
	cmd.AddValue("linkBandwidth", "Link data rate (Default 100 Mbps):", linkBandwidth);
	cmd.AddValue("unitsDataRate", "units of all data rates (Default Mbps):", unitsDataRate);
    ...
    // Currently not passed in on command line
    ...
    avgPacketSize = (minPacketSize + maxPacketSize) / 2; 
	...
	string linkBandwidthXbps  = linkBandwidth  + unitsDataRate;
	string sourceDataRateXbps = sourceDataRate + unitsDataRate;
	string queueSizePkt;
	queueSizePkt = std::to_string(queueSize/2) + "p";

	std::cout << "\t\t---- linkBandwidthXbps= " << linkBandwidthXbps << "=====" << std::endl;;
	...
	// Set link attributes
	p2p.SetDeviceAttribute("DataRate", StringValue(linkBandwidthXbps));
	...
	if (communicationPattern == COMMUNICATION_PATTERN_CUSTOM)
	{
		...
		ifstream myfile("trafficFiles/" + trafficFile + ".txt");
			...
			while (getline(myfile, line))
			{
				if (!line.empty() && line.find(":") != std::string::npos)
				{
					...
					if ((arr[0][0] == 'C' || arr[0][0] == 'c') && (arr[1][0] == 'S' || arr[1][0] == 's'))
					{
						...
						sourceApp.SetAttribute("PacketSize", UintegerValue(avgPacketSize));
						sourceApp.SetAttribute("DataRate", StringValue(sourceDataRateXbps));
	...
	cout << "Traffic source: " << trafficSource << "\n";
	cout << "Burst probability (Default 0): " << p_burst << "\n";
	cout << "Source data rate (Default 0.20 Gbps): " << sourceDataRateXbps << "\n";
	cout << "Source app on state time (Default [Mean=1]): " << sourceOnTime << "\n";
	cout << "Source app off state time (Default [Mean=0]): " << sourceOffTime << "\n";
	...
	// sprintf(fname, "end_to_end_latency.csv", trafficFile.c_str(), (maxPacketSize + minPacketSize) / 2, (maxPacketSize - maxPa
	// 	cketSize) / 2, queueSize, netProtocol.substr(5, 3).c_str(), routing.c_str(), appTime);
	...
	cout << "LinkBandwidth: " << linkBandwidth << "\n";
	cout << "sourceDataRate: " << sourceDataRateXbps << "\n";
	cout << "nFlows: " << nFlows << "\n";
