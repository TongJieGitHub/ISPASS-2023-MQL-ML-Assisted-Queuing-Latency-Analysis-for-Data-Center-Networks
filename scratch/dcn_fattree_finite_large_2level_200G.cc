...
main(int argc, char *argv[]){
	...
	uint64_t sourceMaxBytes = 0;		// unlimited
	double appTime = 10;
	int port = 50001;
	int randomNode = -1;    
	Ptr<Ipv4> ipv4;
	...
	cmd.AddValue ("MaxBytes", "The total number of bytes to send:", sourceMaxBytes);
	...
	sourceDataRate = sourceDataRate + "Gbps";
	...
	vector<double> flow_delaySum, server_delaySum;
	vector<double> flow_avgdelay, server_avgdelay;
	...
	for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator iter = stats.begin (); iter != stats.end (); ++iter)
	{
		...
		flow_delaySum.at(nFlows) = iter->second.delaySum.GetPicoSeconds() / 1000.0;
