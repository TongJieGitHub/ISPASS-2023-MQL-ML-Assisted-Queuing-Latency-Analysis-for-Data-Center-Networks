...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < nFlows; i++)
	{
		...
		if (nFlows <= 16384 || client == 0 || client == 1 || server == 0 || server == 1)
		{
			...
			sprintf(fname, "statistics/statistics_%s_pkt%d_delta%d_queue%d_%s_%s_time%f_200G.csv", trafficFile.c_str(), (maxPacketSize + minPacketSize) / 2, (maxPacketSize - minPacketSize) / 2, queueSize, netProtocol.substr(5, 3).c_str(), routing.c_str(), appTime);
	...
	ofstream of_time;
	sprintf(fname, "statistics/timing_%s_pkt%d_delta%d_queue%d_%s_%s_time%f_200G.csv", trafficFile.c_str(), (maxPacketSize + minPacketSize) / 2, (maxPacketSize - minPacketSize) / 2, queueSize, netProtocol.substr(5, 3).c_str(), routing.c_str(), appTime);
