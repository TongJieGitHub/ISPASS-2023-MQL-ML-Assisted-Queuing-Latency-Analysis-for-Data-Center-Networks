...
int main(int argc, char *argv[])
{
    ...
    int switchRadix;             // Number of ports per switch
	int numApps;				 // Total number of hosts
	bool showIpAddresses = true; // Switch for printing device ip addresses
	bool debug = true;           // Switch for printing debug messages

	// Link parameters
	string linkDataRate = "200";
	...
	cmd.AddValue("dataRate_OnOff", "Source data rate (Default 0.20 Gbps):", sourceDataRate);
	...
	cmd.AddValue("minPacketSize", "Min PacketSize (Default 322):", minPacketSize);
    ...
    // 2-tier non-classical - required inputs
    ...
    numNodesPerEdgeSw = ceil(numNodes / numEdgeSw);

	// Links from a core to an edge - in non-classical can have >1 link between [core_i, edge_j]
    int numLinksPerCoreEdgePair = floor(switchRadix/numEdgeSw);
	...
	string sourceDataRateGbps = sourceDataRate + "Gbps";
	string queueSizePkt;
	...
	cout << "Source data rate (Default 0.20 Gbps): " << sourceDataRateGbps << "\n";
	...
	std::cout << "==============================================" << std::endl;
	std::cout << "In 2-level, no Core-Agg Links created." << std::endl;
	std::cout << "==============================================" << std::endl;
	...
	for (int i = 0; i < nFlows; i++)
	{
		...
		if (nFlows <= 16384 || client == 0 || client == 1 || server == 0 || server == 1)
		{
			...
			sprintf(fname, "statistics/statistics_%s_pkt%d_delta%d_queue%d_%s_%s_time%.0f.csv", trafficFile.c_str(), (maxPacketSize + minPacketSize) / 2, (maxPacketSize - maxPacketSize) / 2, queueSize, netProtocol.substr(5, 3).c_str(), routing.c_str(), appTime);
	...
	ofstream of_time;
	sprintf(fname, "statistics/timing_%s_pkt%d_delta%d_queue%d_%s_%s_time%.0f.csv", trafficFile.c_str(), (maxPacketSize + minPacketSize) / 2, (maxPacketSize - maxPacketSize) / 2, queueSize, netProtocol.substr(5, 3).c_str(), routing.c_str(), appTime);
