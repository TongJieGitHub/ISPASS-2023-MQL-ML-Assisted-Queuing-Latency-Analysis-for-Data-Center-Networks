...
main(int argc, char *argv[]){
			...
			while ( getline (myfile,line) )
			{
				if(!line.empty() && line.find(":") != std::string::npos)
				{
					...
					if ((arr[0][0] == 'C' || arr[0][0] == 'c') && (arr[1][0] == 'S' || arr[1][0] == 's'))
					{
						...
						converter4 >> appStartTime;
						if(appStartTime == 0.0) {
							appStartTime = 0.0;
						}

						stringstream converter5(arr[4].erase(0,1));
						converter5 >> appStopTime;
						if(appStopTime == 0.0) {
							appStopTime = appTime;
						}
						...
						BulkSendHelper sourceApp = BulkSendHelper(netProtocol,Address(InetSocketAddress(Ipv4Address(serverIpAddress), port))); 
						...
						sourceApp.SetAttribute("SendSize",UintegerValue (500));
	...
	for (int index = 0; index < 1; index++) {
		...
		for (int i = 0; i < nFlows; i++) {
			...
			of<<flow_timeFirstTxPacket.at(i).GetNanoSeconds()<<" "<<flow_timeLastRxPacket.at(i).GetNanoSeconds()<<" "<<flow_injection[i]<<" "<<flow_txPackets[i]<<" "<<flow_txBytes[i]<<" "<<flow_rxPackets[i]<<" "<<flow_rxBytes[i]<<" "<<flow_delaySum[i]<<" "<<flow_avgdelay[i]<<" "<< flow_goodput[i] <<" "<< flow_lostPackets[i] <<" "<< numNodes<<endl;

			of.close();

			of2<<flow_timeFirstTxPacket.at(i).GetNanoSeconds()<<" "<<flow_timeLastRxPacket.at(i).GetNanoSeconds()<<" "<<flow_injection[i]<<" "<<flow_txPackets[i]<<" "<<flow_txBytes[i]<<" "<<flow_rxPackets[i]<<" "<<flow_rxBytes[i]<<" "<<flow_delaySum[i]<<" "<<flow_avgdelay[i]<<" "<< flow_goodput[i] <<" "<< flow_lostPackets[i] <<" "<< numNodes<<endl;
	...
	of_time<<elapsed_seconds.count()<<endl;
	of_time.close();
