...
int main(int argc, char *argv[])
{
	...
	cmd.AddValue("tracePath", "path of trace files:", tracePath);
	...
	Config::SetDefault("ns3::TcpSocket::SegmentSize", UintegerValue (1550));
	// traffic control
			...
			while (getline(myfile, line))
			{
				if (!line.empty() && line.find(":") != std::string::npos)
				{
					...
					if ((arr[0][0] == 'C' || arr[0][0] == 'c') && (arr[1][0] == 'S' || arr[1][0] == 's'))
					{
						...
						std::cout << clientNode << " -> " << serverNode << std::endl;
                        ...
                        LoadTraceHelper sourceApp = LoadTraceHelper(netProtocol,Address(InetSocketAddress(Ipv4Address(serverIpAddress), port))); 
						...
						string trace_name = tracePath + "/trace_c" + to_string(clientNode) + "_s" + to_string(serverNode) + ".csv";
						std::cout << "*** " << trace_name << std::endl; 
						sourceApp.SetAttribute("trace_name", StringValue(trace_name));
