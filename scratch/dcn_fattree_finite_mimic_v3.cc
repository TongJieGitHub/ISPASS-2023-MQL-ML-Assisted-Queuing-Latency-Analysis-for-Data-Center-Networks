...
int main(int argc, char *argv[])
{
	...
	float p_burst = 0.0;
	bool ecmp = false;
	string trafficFile = "mimic";
	bool enableLog = false;
	bool enable_first_pod = false;
        string tag;
	...
	cmd.AddValue("enable_first_pod", "enable only first pod log(Default false):", enable_first_pod);
	cmd.AddValue("warmup_ms", " time of warm up", warmup_ms);
	...
	cout << "ecmp: " << ecmp << "\n";
	cout << "time window in us: " << window_ms << "\n";
	cout << "==================================================================="
		 << "\n";
	...
	// Jie
	// stchPfifo.SetRootQueueDisc("ns3::PfifoFastQueueDisc", "MaxSize", StringValue(queueSizePkt));
	...
	if (communicationPattern == COMMUNICATION_PATTERN_CUSTOM)
	{
		...
		if (myfile.is_open())
		{
			...
			cout << "[ERROR] Unable to open file " << traffic_filename << endl;
	}
	else
	{
	...
	monitor->numPods = numPods;
	monitor->enable_first_pod = enable_first_pod;
	monitor->m_TimeWarmUp = MilliSeconds(warmup_ms);
	PeriodicPrintTime ();
	// // Enable pcap
	...
	// Run simulation.
	// NS_LOG_INFO("[ INFO] Running Simulation..");
	// cout << "[ INFO] Running Simulation.." << endl;
	...
	// 		of.open(fname, ios::out | ios::app);
        //     int64_t FCT = flow_timeLastRxPacket.at(i).GetNanoSeconds() - flow_timeFirstTxPacket.at(i).GetNanoSeconds();
	// 		of << sourceDataRate << "," << client << "," << server << "," << s << "," << d << "," << flow_timeFirstTxPacket.at(i).GetNanoSeconds()<<"," << flow_timeLastRxPacket.at(i).GetNanoSeconds() << "," << FCT << "," << flow_injection[i] << "," << flow_txPackets[i] << "," << flow_txBytes[i] << "," << flow_rxPackets[i] << "," << flow_rxBytes[i] << "," << flow_delaySum[i] << "," << flow_avgdelay[i] << "," << flow_goodput[i] << "," << flow_lostPackets[i] << "," << numNodes << "," << randSeed << endl;

	// 		of.close();
	// 	}
	// }

	// Computing the elapsed time of simulation
	auto end_sim = std::chrono::system_clock::now();
	std::chrono::duration<double> elapsed_sim_seconds = end_sim - end_setup;

	// Printing the flow monitor statistics
	cout << "==================================================================="
		 << "\n";
	cout << "Flowmon Stats "
		 << "\n";
	...
	cout << "[ INFO] Simulation finished.." << endl;

	// Printing the elapsed simulation time to file
	ofstream of_time;
    ...
    stats_filename.append(tag);
    stats_filename.append("/reports_sim/");
    stats_filename.append("timing.csv");
	of_time.open(stats_filename, ios::out | ios::app);
	of_time << sourceDataRate << "," << elapsed_setup_seconds.count() << "," << elapsed_sim_seconds.count() << endl;
	of_time.close();

	// Call the simulator destructor
	Simulator::Destroy();
	NS_LOG_INFO("Done.");
	cout << "[ INFO] Simulation complete.." << endl;
