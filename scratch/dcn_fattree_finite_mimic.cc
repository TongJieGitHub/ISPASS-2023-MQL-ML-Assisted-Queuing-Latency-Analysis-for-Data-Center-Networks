...
main(int argc, char *argv[]){
	...
	string queueSize = "8";    // default: 100 packets
	...
	unsigned long sourceMaxBytes = 0;		// unlimited
	double appTime = 10;
	int port = 50001;
	int randomNode = -1;    
	Ptr<Ipv4> ipv4;
	...
	numApps = 200 * numNodes;           // Total number of hosts
	...
	// Jie
  	tchPfifo.SetRootQueueDisc ("ns3::PfifoFastQueueDisc", "MaxSize", StringValue (std::to_string(64) + "p"));
	...
	ApplicationContainer app[numApps];
	int appCount = 0;
	...
	if(communicationPattern == COMMUNICATION_PATTERN_CUSTOM)
	{
		...
		int serverNode;
		int dataRate = 0;
		unsigned long flowSize = 0;
		double appStartTime = 0.0;
		double appStopTime = 0.0;
		std::vector<std::string> arr;
		stringstream converter;
		//int appCount = 0;
		ifstream myfile ("scratch/dcn.traffic");
		if (myfile.is_open())
		{
			while ( getline (myfile,line) )
			{
				if(!line.empty() && line.find(":") != std::string::npos)
				{
					...
					if ((arr[0][0] == 'C' || arr[0][0] == 'c') && (arr[1][0] == 'S' || arr[1][0] == 's'))
					{
						...
						stringstream converter3(arr[2].erase(0,1));
						converter3 >> dataRate;  
						sourceDataRate = std::to_string(dataRate) + "Mbps";

						stringstream converter4(arr[3].erase(0,1));
						converter4 >> flowSize;  
						sourceMaxBytes = flowSize;

						stringstream converter5(arr[4].erase(0,1));
						converter5 >> appStartTime;
						if(appStartTime == 0.0) {
							appStartTime = 0.0;
						}

						stringstream converter6(arr[5].erase(0,1));
						converter6 >> appStopTime;
						if(appStopTime == 0.0) {
							appStopTime = appTime;
						}

						std::cout << clientNode << " -> " << serverNode << ", rate: " << sourceDataRate << ", flowSize: " << flowSize << std::endl;
						...
						sourceApp.SetAttribute("MaxBytes",UintegerValue (sourceMaxBytes));
						...
						app[appCount].Start (Seconds (appStartTime));
						app[appCount].Stop (Seconds (appStopTime));
	...
	std::vector<Ptr<FlowMonitor>> monitorList;
	for(int index = 0; index < 1; index++) {
		monitorList.push_back(flowmon.InstallAll());
		monitorList.at(index)->Start(Seconds(0));
		monitorList.at(index)->Stop(Seconds(10));
	}
	...
	for (int index = 0; index < 1; index++) {
		...
		monitorList.at(index)->CheckForLostPackets ();

		Ptr<Ipv4FlowClassifier> classifier = DynamicCast<Ipv4FlowClassifier> (flowmon.GetClassifier ());
		std::map<FlowId, FlowMonitor::FlowStats> stats = monitorList.at(index)->GetFlowStats ();
		cout << index << " " << stats.size()  << endl;
		...
		vector<double> flow_throughput, server_throughput;
		vector<Time> flow_timeFirstTxPacket;
		vector<Time> flow_timeLastRxPacket;
		...
		for (int i=0; i<appCount; i++)
		{
			...
			flow_throughput.push_back(0);
			flow_timeFirstTxPacket.push_back(NanoSeconds(0));
			flow_timeLastRxPacket.push_back(NanoSeconds(0));
		}
		...
		for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator iter = stats.begin (); iter != stats.end (); ++iter)
		{
			...
			throughput+=(((iter->second.txBytes * 8.0 * 1000 ) / (iter->second.timeLastRxPacket.GetNanoSeconds()-iter->second.timeFirstTxPacket.GetNanoSeconds())) / 1024);

			flow_id.at(nFlows) = iter->first;
			flow_timeFirstTxPacket.at(nFlows) = iter->second.timeFirstTxPacket;
			flow_timeLastRxPacket.at(nFlows) = iter->second.timeLastRxPacket;
			...
			flow_throughput.at(nFlows) = iter->second.rxBytes * 8.0 * 1000 / (iter->second.timeLastRxPacket.GetNanoSeconds()-iter->second.timeFirstTxPacket.GetNanoSeconds());
			flow_avgdelay[nFlows] = flow_delaySum.at(nFlows)/(flow_rxPackets.at(nFlows)+1);

			// flow_id.push_back(iter->first);
			// flow_timeFirstTxPacket.push_back(iter->second.timeFirstTxPacket);
			// flow_timeLastRxPacket.push_back(iter->second.timeLastRxPacket);
			// flow_rxPackets.push_back(iter->second.rxPackets);
			// flow_rxBytes.push_back(iter->second.rxBytes);
			// flow_lostPackets.push_back(iter->second.lostPackets);
			// flow_delaySum.push_back(iter->second.delaySum.GetNanoSeconds());
			// flow_throughput.push_back(iter->second.rxBytes * 8.0 * 1000 / (iter->second.timeLastRxPacket.GetNanoSeconds()-iter->second.timeFirstRxPacket.GetNanoSeconds()));
			// flow_avgdelay.push_back(flow_delaySum[nFlows]/flow_rxPackets[nFlows]);
		...
		string nameAll = "statistics/stat_all.csv";
		ofstream of2;
		of2.open(nameAll, ios::out | ios::app);
		...
		for (int i = 0; i < nFlows; i++) {
			...
			of<<flow_timeFirstTxPacket.at(i).GetMicroSeconds()<<" "<<flow_timeLastRxPacket.at(i).GetMicroSeconds()<<" "<<flow_rxPackets[i]<<" "<<flow_rxBytes[i]<<" "<<flow_delaySum[i]<<" "<<flow_avgdelay[i]<<" "<< flow_throughput[i] <<" "<< flow_lostPackets[i] <<" "<< numNodes<<endl;

			of.close();

			of2<<flow_timeFirstTxPacket.at(i).GetMicroSeconds()<<" "<<flow_timeLastRxPacket.at(i).GetMicroSeconds()<<" "<<flow_rxPackets[i]<<" "<<flow_rxBytes[i]<<" "<<flow_delaySum[i]<<" "<<flow_avgdelay[i]<<" "<< flow_throughput[i] <<" "<< flow_lostPackets[i] <<" "<< numNodes<<endl;


		}
		of2.close();

		monitorList.at(index)->SerializeToXmlFile(filename, true, true);
