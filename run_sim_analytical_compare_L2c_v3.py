...
def run_simulator (argument_tuple) :
    ...
    if 'mimic' not in str(config_data['SimulationParamsInputs']['trafficFile']) :
        if 'UDP' in str(config_data['SimulationParamsInputs']['netProtocol']) :
            additional_parameters = "--netProtocol='ns3::UdpSocketFactory'" 
        else : 
    ...
    os.system("./waf --run-no-build=\"" + \
        ...
        "--linkBandwidth=" + str(config_data['SimulationParamsInputs']['linkBandwidth']) + " " + \
        "--unitsDataRate=" + str(config_data['SimulationParamsInputs']['unitsDataRate']) + " " + \
...
#print(str(config_data['SimulationParamsInputs']['simTime_inms']))
...
merge_latency_per_flow(tag+"_merged", sim_flow_filename, ana_flow_filename, latency_per_flow_filename)
merge_latency_per_queue(tag+"_merged", sim_queue_filename, ana_queue_filename, latency_per_queue_filename)
